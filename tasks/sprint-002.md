...
Implement API endpoints for post likes and efficient view counting mechanism.
...
1. **Post Likes**
   ...
   - Track total likes count for each post
   - Get user's liked status for posts

2. **View Tracking**
   - Track view counts for published posts
   - Handle high-frequency view events efficiently
   - Prevent duplicate views from same user in short time periods
   - Provide accurate view counts for posts
...
1. **Data Models**
   ...
   model PostView {
     id         String   @id @default(cuid())
     postId     String
     viewerId   String?  // userId if logged in, null if anonymous
     viewerHash String   // Hash of IP + UserAgent for anonymous users
...
2. **API Endpoints**
   ...
   // Record view (internal endpoint)
   POST /api/v1/posts/:id/view
   {
     viewerId?: string;  // For logged-in users
     viewerHash: string; // For anonymous users
   }
   ```

3. **View Counting Strategy**
   - Use Redis for real-time view counting
   - Implement view deduplication with TTL
   - Batch process views to database
   - Aggregate views periodically

### Technical Notes

1. **View Counting Implementation**
   - Use Redis HyperLogLog for unique viewer counting
   - Set 30-minute cooldown per viewer per post
   - Background job to sync Redis counts to database
   - Cache view counts in Redis

2. **Repository Layer**
   - Implement PostLikeRepository
   - Implement PostViewRepository
   - Use Redis for view counting operations

3. **Service Layer**
   - Implement LikeService
   - Implement ViewService with Redis integration
   - Handle view deduplication logic

4. **Performance Optimizations**
   - Batch write views to database
   - Cache like counts
...
2. Set up Redis for view counting
3. Implement like functionality
4. Implement view tracking system
5. Create background jobs for view processing
6. Add caching layer
7. Write tests
...
- 404: Post not found
- 403: Not authorized to like
- 429: Too many view requests
...
Medium - Important for content engagement metrics
