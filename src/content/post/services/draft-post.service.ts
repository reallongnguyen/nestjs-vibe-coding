...
export class DraftPostService {
  ...
  async updateDraftPost(
    ...
    this.logger.debug(`Updating draft post ${id} for user ${userId}`);
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        // Validate topics if provided
        if (data.topics?.length) {
          const existingTopics = await topicRepo.findManyByIds(data.topics);
          if (existingTopics.length !== data.topics.length) {
            const foundTopicIds = existingTopics.map((t) => t.id);
            const missingTopicIds = data.topics.filter(
              (topicId) => !foundTopicIds.includes(topicId),
            );
            throw ContentErrorFactory.topicNotFound(missingTopicIds);
          }
        }
        ...
        // Update draft post
        const updated = await draftRepo.update(id, data);

        // Publish events
        try {
          await this.events.publishPostUpdatedEvent(updated);
        } catch (error) {
          this.logger.error(
            `Failed to publish post updated event: ${error.message}`,
            error.stack,
          );
        }

        // Delete removed images (not in a transaction as it's an async operation)
        if (removedImages.length > 0) {
          try {
            // Use Promise.all to avoid await in loop
            await Promise.all(
              removedImages.map((image) =>
                this.eventBus.publish(new DeleteImageCommand(image)),
              ),
            );
          } catch (error) {
            this.logger.error(
              `Failed to delete removed images: ${error.message}`,
              error.stack,
            );
          }
        }

        this.logger.debug(`Updated draft post ${id}`);
